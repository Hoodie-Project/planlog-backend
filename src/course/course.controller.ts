...
export class CourseController {
  ...
  @ApiOperation({
    ...
    description: [
      ...
      '4. **시간표 배치** — `startTime`(기본 10:00) 시작, 구간마다 (거리÷이동속도) 이동시간 + 체류시간 누적, 13시 이후 점심 자동 삽입',
      '5. **부가 정보** — `travelDate` 주면 그 요일 혼잡도+한산요일 추천, `debug` 주면 연관 매칭 적중률 진단',
      '',
      '### 규칙값',
      '| 항목 | 값 |',
      '|---|---|',
      '| 한 구간 최대 이동거리 | 뚜벅이 2.5km / KTX 6km / 렌터카 30km (FAMILY 는 60%로 축소) |',
      '| 이동 속도 | 도보 4 / KTX 30 / 렌터카 40 (km/h) |',
      '| 체류 시간 | 관광지 90분(FAMILY 는 120분) · 점심 60분 |',
      '',
      '### 요청 본문(JSON)',
      '- `zone`(필수): SEA(동해바다)/SNOW(설원산악)/VALLEY(계곡자연)/RETRO(레트로문화)/PHOTO(절경포토)',
      '- `transport`: WALK(뚜벅이·기본)/KTX/CAR(렌터카) — 동선 반경이 달라짐',
      '- `style`: SOLO(기본) / FAMILY(가족과 함께 — 구간 축소·체류시간 확대) / PET(반려동물과 함께 — 펜션·캠핑 가중) / CALM(조용히 쉬고 싶어요 — 인기(연관 다발) 스팟 감점, 한적한 곳 우선)',
      '- `spotCount`: 하루당 관광지 수(2~5, 기본 3)',
      '- `nights`: 0=당일/1=1박2일/2=2박3일 (기본 0)',
      '- `travelDate`(선택, YYYY-MM-DD): 혼잡도 안내 추가',
      '- `startTime`(선택, HH:mm 24시간제): 1일차 시작(도착) 시각, 기본 10:00',
      '- `startMapX`/`startMapY`(선택): 1일차 출발 좌표(예: KTX역·버스터미널 — GET /stations 참고)',
